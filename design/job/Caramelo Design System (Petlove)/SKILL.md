...
- **Type:** Inter (body/UI), Gooper (display — real licensed face in `assets/fonts/`), Roboto Mono. Sentence case.
- **Shape:** pill buttons/chips/inputs; 16–20px cards; rounded & friendly.
- **Tokens:** `tokens/*.css` (curated `--c-*`, `--fs-*`, `--space-*`, `--radius-*`, `--shadow-*`) + `tokens/fig-tokens.css` (full Figma export).
- **Components:** `components/<group>/` → `window.CarameloDesignSystemPetlove_6f57df`. Icons via `Icon` (currentColor). Logos in `assets/brand/`.
- **Reference UI:** `ui_kits/petlove-shop/` interactive storefront.
