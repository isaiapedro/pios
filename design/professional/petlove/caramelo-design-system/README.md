...
**Type.** Two families: **Inter** for all body and UI text, and **Gooper** (a rounded, friendly slab serif by Latinotype) for display/brand headings. Gooper is shipped as the real licensed face (`shared/assets/fonts/`, weights SemiBold 600 / Bold 700 / Black 900). Mono is **Roboto Mono**. Scale: 12 / 14 / 16 / 22 / 24 / 32 / 48. Headings are bold; body is regular; labels are medium.
...
- This project ships **21 real extracted glyphs** through a single `Icon` component (`shared/components/icons/`): arrow-right, cart, cat, check, chevron-down/left/right, close, close-circle, danger, dog, gift, heart, home, info, minus-circle, plus, search, star, success, warning. Glyph path data lives in `shared/components/icons/iconRegistry.js`.
...
- `shared/tokens/colors.css` · `typography.css` · `spacing.css` · `fonts.css` · `base.css` — curated, human-readable token layer (use `--c-*`, `--fs-*`, `--space-*`, `--radius-*`, `--shadow-*`).
- `shared/tokens/fig-tokens.css` — full Figma variable export (all primitives + semantic aliases + theme modes) for completeness.
- `shared/guidelines/*.card.html` — foundation specimen cards (colors, type, spacing, radii, shadows).

**Components** (`shared/components/<group>/`, exposed on `window.CarameloDesignSystemPetlove_6f57df`)
