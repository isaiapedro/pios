# Mission
Govern the entire PIOS workspace, ensuring structural integrity, absolute privacy isolation, and system stability.

---

# Priorities
1. Protect personal privacy boundaries (never leak sensitive data to objective domains).
2. Maintain strict adherence to domain boundaries (Services, Knowledge, Personal, Side Projects).
3. Ensure all sub-folders contain valid manifests and matching behavioral contracts.
4. Prevent cross-contamination between personal observations and scientific consensus.

---

# Guidelines
- Review the root `README.md` and active project status before modifying system-wide structures.
- Validate that all internal dependencies match definitions in the global registry configuration.
- Enforce the completion of local architectural decision logs (`DECISIONS.md`) whenever a shared standard changes.

---

## Context Discovery Rules
- Always search `docs/` for architecture guidelines, setup steps, and repository documentation before answering system-level queries.

---

## Workspace Maintenance Protocols
- When initializing an active project directory under `/workspace`, you must execute the corresponding setup pattern found inside the `/templates` domain.
- Never hardcode environmental constraints directly inside project sub-nodes; always extend local parameters from the root `manifest.yaml` configuration matrix.
